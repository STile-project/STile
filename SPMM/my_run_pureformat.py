...
names = ['pruned_bert', 'pruned_bert_unstructured'] + ['cora', 'ppi', 'pubmed',  'arxiv', 'proteins', 'reddit', 'citeseer'][::-1]
names = ['cora', 'ppi', 'pubmed',  'arxiv', 'proteins', 'reddit', 'citeseer'][::-1]
...
for (only_TC, only_ELL) in [(False, True)]: # [(True, False), (False, True)]:
	for name in names:
		tot_num = 1
		get_op = None
		...
		for data_i in range(tot_num):		
			for feat_size in [32, 512]: #[32, 64, 128, 256, 512]:
			...
			# for max_avg_cost_diff in [0.2]: #[float('inf')]: # [0.2]: #[0, 0.2, 0.4, 0.6]:
				...
				print(f"name: {(name, data_i)}  feat_size: {feat_size}, only_TC: {only_TC}")
