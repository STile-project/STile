...
def bench_bsrmm(bsr_mat: Any, x: th.Tensor, block_size: int, filename: str, data_i: int, name = 'structured', dim = 512):
    global bsrmm
    ...
    with open(filename, 'a') as file:
        json.dump(['bsrmm', (name, data_i), dim, ('time', avg_time), ('mem', get_memory_usage(args))], file)
        file.write('\n')

    return avg_time


def bench_dbsrmm(bsr_mat: Any, x: th.Tensor, block_size: int, filename: str, data_i: int, name = 'structured', dim = 512):
    global bsrmm
    mb = bsr_mat.shape[0] // bsr_mat.blocksize[0]
    nb = bsr_mat.shape[1] // bsr_mat.blocksize[1]
    nnzb = bsr_mat.nnz // (block_size**2)
    ...
    sch.compute_at(B_local, bjo)
    print("before hide buffer access!!!!!!!")
    print(sch.mod.script())
    sch.hide_buffer_access(blk_inner, "read", [1, 4])
    print(sch.mod.script())

    sch.blockize(sch.get_loops(blk_inner)[-3])
    print("after blockize !!!!!!!!!")
    print(sch.mod.script())
    ...
    mod = lower_sparse_buffer(sch.mod)
    print("after lower sparse buffer in the end.")
    print(mod.script())
    ...
    with open(filename, 'a') as file:
        json.dump(['dbsrmm', (name, data_i), dim, ('time', avg_time), ('mem', get_memory_usage(args))], file)
        file.write('\n')

    return avg_time


def bench_cublas(W: th.Tensor, X: th.Tensor, filename: str, data_i: int, name = 'structured', dim = 512):
    ...
    with th.no_grad():
        ...
        with open(filename, 'a') as file:
            json.dump(['cublas', (name, data_i), dim, ('time', measure), ('mem', memory_usage)], file)
            file.write('\n')

        return measure


def bench_cusparse(csr: Any, X: th.Tensor, filename: str, data_i: int, name = 'structured', dim = 512):
    th.cuda.reset_peak_memory_stats()
    ...
    with th.no_grad():
        ...
        with open(filename, 'a') as file:
            json.dump(['cusparse', (name, data_i), dim, ('time', measure), ('mem', memory_usage)], file)
            file.write('\n')

        return measure


def bench_triton(bsr: Any, X: th.Tensor, block_size: int, filename: str, data_i: int, name = 'structured', dim = 512):
    indptr = bsr.indptr
    indices = bsr.indices
    M, K = bsr.shape
    ...
    with open(filename, 'a') as file:
        json.dump(['triton', (name, data_i), dim, ('time', measure), ('mem', memory_usage)], file)
        file.write('\n')
...
def do_profile_structured(dim, csv, filename):
    ...
    for name, param in model.named_parameters():
        if (
            ...
            sparsetir_durs.append(bench_bsrmm(bsr_weight, x, 32, filename, count, dim=dim))
            sparsetir_dbsrmm_durs.append(bench_dbsrmm(bsr_weight, x, 32, filename, count, dim=dim))
            cublas_durs.append(bench_cublas(param.data, x, filename, count, dim=dim))
            cusparse_durs.append(bench_cusparse(csr_weight, x, filename, count, dim=dim))
            triton_durs.append(bench_triton(bsr_weight, x, 32, filename, count, dim=dim))
            count+=1
...
def do_profile_sparse_attention(old_dim, csv, filename):
    names = ['strided', 'logsparse']
    count = 0 # count is used as data_i
    block_size = 16 # 32
    for name in names:
        for dim in [32, 64, 128, 256, 512]:
            op = get_op('spmm', 0, dim, name, m=4096)
            csr_weight = op.inps[0]
            # bsr_weight = csr_weight.tobsr(blocksize=(32, 32))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", "-n", type=str, default='structured', help="the dataset name")
    ...
    if args.name == 'structured':
        filename = "Mem_Baseline_Structured_fp16.json"
        with open(filename, 'a') as file:
            file.write(f"\n\n\n\nNew Round---------\n")
        do_profile_structured(args.dim, args.csv, filename)
    else:
        filename = "Mem_Baseline_SAttention_fp16.csv"
        filename = "Mem_Triton16_SAttention_fp16.csv"
        with open(filename, 'a') as file:
            file.write(f"\n\n\n\nNew Round---------\n")
        do_profile_sparse_attention(args.dim, args.csv, filename)
