...
def bench_bsrmm(bsr_mat: Any, x: th.Tensor, block_size: int, filename: str, data_i: int, name='unstructured', dim=512):
    global bsrmm
...
def bench_tc_spmm(sp_mat: Any, x: th.Tensor, mma_shape_str: str, filename: str, data_i: int, name='unstructured', dim=512):
    ...
    with open(filename, 'a') as file:
        json.dump(['tc-spmm', (name, data_i), dim, ('time', avg_time), ('mem', get_memory_usage(args))], file)
        file.write('\n')
...
def bench_cublas(W: th.Tensor, X: th.Tensor, filename: str, data_i: int, name='unstructured', dim=512):
    with th.no_grad():
        W = W.half().to(0)
        X = X.to(0)
...
def bench_cusparse(csr: Any, X: th.Tensor, filename: str, data_i: int, name='unstructured', dim=512):
...
def do_profile_unstructured(dim, csv, filename):
    ...
    for name, param in model.named_parameters():
        if name.endswith("dense.weight") or name.endswith(
            ...
            sparsetir_sr_bcrs_durs.append(
                bench_tc_spmm(csr_weight, x, "m8n32k16", filename, count, dim=dim))
            sparsetir_bsrmm_durs.append(bench_bsrmm(bsr_weight, x, 32, filename, count, dim=dim))
...
def do_profile_sparse_attention(old_dim, csv, filename):
    ...
    for name in names:
        for dim in [32, 64, 128, 256, 512]:
            ...
            # 
            sparsetir_sr_bcrs_durs = (
                bench_tc_spmm(csr_weight, x, "m8n32k16", filename, count, name, dim=dim))
            sparsetir_bsrmm_durs = (bench_bsrmm(bsr_weight, x, 32, filename, count, name, dim=dim))
            # cublas_durs = (bench_cublas(param.data, x, filename, count, name, dim=dim))
            cusparse_durs = (bench_cusparse(csr_weight, x, filename, count, name, dim=dim))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", "-n", type=str, default='unstructured', help="the dataset name")
    ...
    if args.name == 'unstructured':
        filename = "Mem_Baseline_Unstructured_fp16.json"
        with open(filename, 'a') as file:
            file.write(f"\n\n\n\nNew Round---------\n")
        do_profile_unstructured(args.dim, args.csv, filename)
    else:
