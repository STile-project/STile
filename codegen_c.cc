...
void CodeGenC::AddFunction(const PrimFunc& f) {
  ...
  // <fjz debug begin>
  // std::cout << "total number of parameters: " << f->params.size() << '\n';
  bool find_shared_input = false;
  size_t vi_shared = f->params.size();
  // <fjz debug end>
  ...
  for (size_t i = 0; i < f->params.size(); ++i) {
    ...
    // <fjz debug begin>
    std::string shared_const = "shared";
    if (vid.find(shared_const)!=std::string::npos){
      find_shared_input = true;
      stream << "haha";
      vi_shared = i;
    }

    // std::cout << "parameter name: " << vid << '\n';
    // <fjz debug end>
  ...
  // <fjz debug begin>
  if (find_shared_input){
    stream << "__shared__ ";
    PrintType(GetType(f->params[vi_shared]), stream);
    stream <<" A_shared[24576];\n";
  }
  // <fjz debug end>
